...
<div class="h-9 px-6 border-b border-[#d4d4d4] flex items-center">
...
<div class="flex-1 flex items-center justify-center p-6">
  <div class="w-full max-w-sm space-y-4">
    <div>
      <span class="block text-xs text-[#6b6b6b] mb-1.5">name</span>
      <Input bind:value={formData.name} placeholder="my-redis" />
    </div>
    
    <div class="grid grid-cols-2 gap-3">
      <div>
        <span class="block text-xs text-[#6b6b6b] mb-1.5">host</span>
        <Input bind:value={formData.host} placeholder="127.0.0.1" />
      </div>
      <div>
        <span class="block text-xs text-[#6b6b6b] mb-1.5">port</span>
        <Input type="number" bind:value={formData.port} />
      </div>
    </div>

    <div>
      <span class="block text-xs text-[#6b6b6b] mb-1.5">username <span class="text-[#9a9a9a]">(optional)</span></span>
      <Input bind:value={formData.username!} placeholder="default" />
    </div>

    <div>
      <span class="block text-xs text-[#6b6b6b] mb-1.5">password <span class="text-[#9a9a9a]">(optional)</span></span>
      <Input type="password" bind:value={formData.password!} />
    </div>

    <div>
      <span class="block text-xs text-[#6b6b6b] mb-1.5">database</span>
